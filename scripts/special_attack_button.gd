...
var damages = [2, 3, 5, 7, 10, 14, 18, 24, 30, 40]
...
func _on_pressed() -> void:
	if game_manager.score >= costs[level]: # checks if you can afford the upgrade
		...
		level += 1
		dracula.special_bat_count = bats[level] # updates dracula's attack cooldown
		dracula.special_bat_damage = damages[level] # updates dracula's attack cooldown
		update()
